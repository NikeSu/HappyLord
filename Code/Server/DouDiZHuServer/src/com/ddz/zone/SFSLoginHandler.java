...
public class SFSLoginHandler extends BaseServerEventHandler {
	...
	public void handleServerEvent(ISFSEvent event) throws SFSException {
		...
		//使用数据库根据用户名进行查询
		List<HashMap> accounts=DBManager.GetInstance().doQuery("select * from user where username='"+name+"'");
				               //Global.db.doQuery("select * from user where username='"+name+"'");
		...
		if(accounts.size()<1) {
			...
			if(!this.getApi().checkSecurePassword(session, user.get("userpwd").toString(),pwd)) {
				SFSErrorData data=new SFSErrorData(SFSErrorCode.LOGIN_BAD_PASSWORD);
				data.addParameter(user.get("userpwd").toString());
